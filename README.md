...
The API has a built-in seeder method. On initialization, the database of library items will be replaced with the data in /src/seed.ts. This is called in the constructor function of the itemsService class, and can be removed if no longer needed.

## Using the API

RESTful Routes:

The API is set to run on 'localhost:3000', unless a port is otherwise specified in process.env
